...
class VSphereMockHttp(MockHttp):
    ...
    def _rest_vcenter_cluster(self, method, url, body, headers):
        if method == "GET":
            body = self.fixture.load('list_clusters.json')
        else:
            raise AssertionError('Unsupported method')
        return (httplib.OK, body, {}, httplib.responses[httplib.OK])
    ...
    def _rest_vcenter_host(self, method, url, body, headers):
        if method == "GET":
            body = self.fixture.load('list_hosts.json')
        else:
            raise AssertionError('Unsupported method')
        return (httplib.OK, body, {}, httplib.responses[httplib.OK])

    def _rest_vcenter_vm_VMID_power_stop(self, method, url, body, headers):
        if method != "POST":
            raise AssertionError('Unsupported method')
        body = ""
    ...
    def _rest_vcenter_vm_VMID_power_start(self, method, url, body, headers):
        if method != "POST":
            raise AssertionError('Unsupported method')
        body = ""
    ...
    def _rest_vcenter_vm_VMID_power_reset(self, method, url, body, headers):
        if method != "POST":
            raise AssertionError('Unsupported method')
        body = ""
